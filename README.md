# qa-portfolio

## Project Overview

This repository contains manual QA test documentation for a web application, including test cases and bug reports for authentication and form validation features.
The project demonstrates practical QA skills in test design, defect reporting, and structured documentation using Markdown and GitHub. It focuses on validating functional behavior, input constraints, and UI interactions in a real-world testing scenario.

The goal of this project is to demonstrate practical knowledge of:

- Manual test case design
- Boundary Value Analysis
- Equivalence Partitioning
- Positive and negative testing
- UI and functional validation
- Test documentation in Markdown format
- GitHub repository organization

---

## Test Coverage

This project includes test cases for the following areas:

- Login functionality
- Registration functionality
- Payment form validation
- Field validation (including boundary value analysis)
- UI elements and button behavior

Each module focuses on realistic web application testing scenarios.

---

## Testing Techniques Used

The following QA techniques were applied:

- Boundary Value Analysis
- Equivalence Partitioning
- Positive testing 
- Negative testing 
- UI validation testing

---

## Project Structure

```
/test-cases
|
├── authentication
|   ├── login_tests.md
|   └── registration_tests.md
├── form_validation
|   └── payment_form_validation.md
|
/bug-reports
|
├── login_bugs.md
├── payment_form_bugs.md
└── registration_bugs.md
|
README.md
```

Each folder contains Markdown files grouped by application feature for better readability and scalability.

---

## Test Case Format

Each test case follows a consistent structure:

- Test Case ID (TC-001, TC-002, etc.)
- Title / Description
- Steps
- Expected Result

Example:

**TC-001 – Valid input scenario**

Steps:
1. Enter valid data
2. Click action button

Expected Result:
System processes input successfully.

---

## Bug Reports

Bug reports are documented separately for each feature module.

Each bug report follows a consistent structure:

- Bug report ID (BR-001, BR-002, etc.)
- Related Test Case
- Issue
- Reproduction Steps
- Expected Result
- Actual Result
- Severity


---

## Purpose of This Project

This repository was created as part of a QA learning journey to demonstrate the ability to:

- Design structured test cases
- Apply testing techniques to real-world scenarios
- Organize test documentation clearly
- Use GitHub for professional portfolio presentation

---

## Author

Natalia 

Aspiring QA Engineer focused on manual testing and test automation fundamentals.
